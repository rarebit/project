...
Rarebit Protocol v0.2 - JS implementation
  ...
  v0.1 spec, they are hashed to form CIDs, which are now called refIDs).
    ...
    refID: bitcoin address calculated from RefKey in the normal way
...
/*
  Create origination transaction for refkey (issue lots)
    ...
    memo: text to add to tx, such as a url (36 bytes max)
*/
Rarebit.Tx.createOrigination = function( 
                             refkey, issuerKey, qtys, seedWallet, fee, memo ) {
  if (!issuerKey)
  ...
  if (memo)
    outs.push( Rarebit.Tx.prepMemoOut(Rarebit.Tx.memoTypeOrigination,memo) );
...
/*
  Create transfer transaction (transfer lots)
    ...
    memo: text to add to tx, such as a url (36 bytes max)
*/
Rarebit.Tx.createTransfer = function( refkey, lots, newOwners, seed, fee, memo ) {
  ...
  try {
    ...
    if (memo)
      outs.push( Rarebit.Tx.prepMemoOut(Rarebit.Tx.memoTypeTransfer,memo) );
...
/*
  Prep a Rarebit memo output definition (for use by Bitcoin.Wallet.createSend2)
    memo: text such as a url
*/
Rarebit.Tx.memoPrefix = 'rb0';
Rarebit.Tx.memoSeperator = ' ';
Rarebit.Tx.prepMemoOut = function( txtyp, memo ) {
  while (memo.substr(0,1) == ' ') memo = memo.substr( 1 );
  return { Data: {memo:Rarebit.Tx.memoPrefix + Rarebit.Tx.memoSeperator + memo} };
}
Rarebit.Tx.getMemo = function( outText ) {
  var fp = Rarebit.Tx.memoPrefix + 
           Rarebit.Tx.memoSeperator;
  return outText ? outText.substr( fp.length ) : "";
}
...
/*
  Find and verify all unspent rarebit outputs containing refID
    db: database (wallet) containing txs
    ...
    strict: if true, txs referenced by inputs must be in db
    returns: {lots:[],invalidlots:[],ttlqty:<>};
      lot: {txhash:<>,txoutindex:<>,provenance:{}}
        provenance: result of .traceLotProvenance()
*/
Rarebit.Tx.queryLots = function( db, refID, issuer, owner, strict ) {
  ...
  for( var i=0,oi,trace,vs; i<outs.outsStats.length; i++ ) {
    ...
    if (!owner || owner == oi.addrs[1].toString()) {
      ...
      if (trace.memo) vs.memo = trace.memo;
      //if (trace.txtimestamp) vs.txtimestamp = trace.txtimestamp;
      if (trace.err)
...
/*
  Trace a lot's provenance
    db: database (wallet) that contains transactions
    ...
    strict: if true, txs not found are considered errors, not endpoints
...
Rarebit.Tx.traceLotProvenance = function( 
                         dbt, txhash, outindex, oissuer, orefID, strict ) {
  var db = dbt;
  ...
  function err_( r, msg ) {
    ...
    ret.recerr = msg;
    ret.err = "Trace failed, record " + r.recnum + " " + "[" + msg + "]";
  }
      ...
      if (!Rarebit.Tx.verifyOutput( oo ))
        ...
        if (refID != oo.addrs[0].toString())
          ...
          if (!s.outsStats.length)
            ...
            //  get memo attachment if available
            var memo = db.queryOutputs( 'allOuts', 
                              {txHash:txh,eqmemo:Rarebit.Tx.memoPrefix} );
            if (memo.outsStats.length)
              r.memo = Rarebit.Tx.getMemo( memo.outsStats[0].memo );
  ...
  if (!ret.err) ret.txtimestamp = ret.trace[0].txtimestamp;
  if (!ret.err && ret.trace[0].memo) ret.memo = ret.trace[0].memo;
  return ret;
}


/*
  Get array of refIDs in tx database
*/
Rarebit.Tx.extractRefIDs = function( db ) {
  var outs = db.queryOutputs( 'allOuts', {eqM:2, eqN:2} );
  for (var i=0, rid, rids=[]; i<outs.outsStats.length; i++ ) {
    rid = outs.outsStats[i].addrs[0];
    if (rid.findIn( rids ) < 0)
      rids.push( rid.toString() );
  }
  return rids;
}
...
*/
Rarebit.Tx.cleanQty = function( q, minq ) {
  minq = minq ? minq : '0';
  if (!q)
    q = minq;
  if (typeof(q) == 'string')
    ...
    q = q.toString();
  while (q.substr(0,1) == '0') q = q.substr( 1 );
  if (!q || q == '0')
    q = minq;
  return q;
}
...
/*
  HashKey, special case of refKey where key is a hash of some content
*/
