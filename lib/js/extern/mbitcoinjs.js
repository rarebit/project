...
*/
Bitcoin.Address.mine = function( matches, callback, dictCallback ) { 
  ...
  Bitcoin.Address.__minerProcesses[pid] = 
             {'matches':matches,'callback':callback,dict:dictCallback,i:0};
  callback( "progress", 0 );
  setTimeout( 'Bitcoin.Address.__minerStep('+pid+')', 
              Bitcoin.Address.__minerProcesses.delay );
}
...
*/
Bitcoin.Address.__minerProcesses = {nextpid:1,delay:1};
Bitcoin.Address.__minerStep = function( pid ) {
  ...
  if (p) {
    ...
    var k = Bitcoin.Address.fromPrivOrPass( "", p.dict );
    if (!isin( k, p.matches )) {
