...
//    auto-detects whether BCI-unspents or BBE data
//
//  status = Bitcoin.ImpExp.BBE.export( wallet )
//    status: {text:<JSON>,txsRejected:<>,txsAccepted:<>}
//
//  Bitcoin.ImpExp.Sync.loadAddrs( wallet, callbacks, addresses, pfx )
//    loads data into a wallet from BCI or BBE server
//
...
/*
  Loader, connects, downloads transactions, adds them to wallet
    ...
    callbacks: onprogress( n, address, % ), 
               onaddrloaded( n, address, status, wallet ),
               ontxloaded( n, tx, status, wallet ),
               onerror( n, address, msg ), oncomplete( wallet )
                 n: address being loaded, address: address str, stat: from import
*/
Bitcoin.ImpExp.Sync = {};
Bitcoin.ImpExp.Sync.BCIUnspentsURL = "https://blockchain.info/unspent?address=";
Bitcoin.ImpExp.Sync.BBEAddrURL = 'https://blockexplorer.com/q/mytransactions/';
Bitcoin.ImpExp.Sync.BBETxURL = 'https://blockexplorer.com/rawtx/';
Bitcoin.ImpExp.Sync.BBETxURL_testnet = 'https://blockexplorer.com/testnet/rawtx/';
Bitcoin.ImpExp.Sync.BIOAddrURL = 'http://btc.blockr.io/api/v1/address/txs/';
Bitcoin.ImpExp.Sync.BIOAddrURL_testnet = 'http://tbtc.blockr.io/api/v1/address/txs/';


/*
  load tx data for addresses from URL prefix (default pfx=BBE)
*/
Bitcoin.ImpExp.Sync.loadAddrs = function( 
                                  wallet, callbacks, addresses, pfx, _testnet ) {
  function fin() {
  ...
  function ontl( t ) {
    ...
    if (_testnet) {
      var cb = {
        oncomplete: function (w,nin) {txsin+=nin;if(ac==addresses.length)fin();},
        ontxloaded: function (stat,w) {},
        onerror: function (e) {onte(e);}
      }
      var m = Bitcoin.ImpExp.Sync.processBIOMasterList( t );
      if (m.length == 0 && ac == addresses.length)
        return fin();
      Bitcoin.ImpExp.Sync.loadTxs( wallet, cb,
                        m, 
                        Bitcoin.ImpExp.Sync.BBETxURL_testnet );
      return;
    }
    var status = Bitcoin.ImpExp.BBE.import( t, wallet );
    //stat = {txsRejected:<>,txsAccepted:<>}
    txsin += status.txsAccepted;
    if (callbacks.onaddrloaded)
      callbacks.onaddrloaded( status, wallet );
  }
  function onte( e ) {
    if (callbacks.onerror)
      callbacks.onerror( e );
  }
  var a, b64 = false, txsin = 0, ac = 0;
  pfx = pfx ? pfx : Bitcoin.ImpExp.Sync.BBEAddrURL;
  if (_testnet)
    pfx = Bitcoin.ImpExp.Sync.BIOAddrURL_testnet;
  if (!wallet)
  ...
  callbacks.ontexterror = onte;
  var urls = Bitcoin.ImpExp.Sync.createAddrURLs( pfx, addresses, b64, _testnet );
  Bitcoin.ImpExp.Sync.loadURLs( callbacks, urls );
}


/*
  load tx data from URL prefix (default pfx=BBE)
*/
Bitcoin.ImpExp.Sync.loadTxs = function( wallet, callbacks, txs, pfx ) {
  function fin() {
    if (callbacks.oncomplete)
      callbacks.oncomplete( wallet, txsin );
  }
  function ontl( t ) {
    var status = Bitcoin.ImpExp.BBE.import( '{"1":' + t + '}', wallet );
    txsin += status.txsAccepted;
    if (callbacks.ontxloaded)
      callbacks.ontxloaded( status, wallet );
  }
  function onte( e ) {
    if (callbacks.onerror)
      callbacks.onerror( e );
  ...
  var txsin = 0;
  pfx = pfx ? pfx : Bitcoin.ImpExp.Sync.BBETxURL;
  if (!wallet)
    wallet = new Bitcoin.Wallet();
  callbacks.ontextloaded = ontl;
  callbacks.ontexterror = onte;
  callbacks.onallloaded = fin;
  var urls = Bitcoin.ImpExp.Sync.createTxURLs( pfx, txs );
  Bitcoin.ImpExp.Sync.loadURLs( callbacks, urls );
}


/*
  extract tx hashes from BIO master list
*/
Bitcoin.ImpExp.Sync.processBIOMasterList = function( t ) {
  //{ data:{ txs:[{tx:"fffff"},{tx:"aaaaa"}] } };
  var txs = [];
  t = JSON.parse( t );
  for( var i=0; i<t.data.txs.length; i++ )
    txs.push( t.data.txs[i].tx );
  return txs;
}
...
*/
Bitcoin.ImpExp.Sync.loadURLs = function( callbacks, urls ) {
  function ontl( t ) {
    if (next >= 0 && ontl_)
      ontl_( t );
    next++;
    if (next < urls.length)
      Bitcoin.ImpExp.Sync.loadURL( callbacks, urls[next].url );
    else
...
/*
  load a url, get text
*/
Bitcoin.ImpExp.Sync.loadURL = function( callbacks, url ) {
  ...
  function onProgress( e ) {
    if (e.lengthComputable)
      if (callbacks.onprogress)
        callbacks.onprogress( (e.loaded/e.total)*100 );
  }
  function transferComplete( e ) {
    ...
    if (i1 < 0 || i2 < 0)
      if (callbacks.onerror)
        return callbacks.onerror( i, data, "Invalid response" );
    i1 += 3;
  ...
  function transferFailed( ) {
    ...
    // if (callbacks.ontextloaded) callbacks.ontextloaded( i, data, "" );
    if (callbacks.ontexterror)
      callbacks.ontexterror( m );
  }
  function transferCanceled( e ) {
    var s = this.status;
    if (callbacks.ontexterror)
      callbacks.ontexterror( "Download canceled" );
  }
...
/*
*/
Bitcoin.ImpExp.BCI.sendNewTx = function( tx ) {
/* TO DO
