...
//
//BlockExplorer implementation (code below, adapted from BrainWallet);
//
...
Bitcoin.ImpExp.BBE.importTx = function( tx ) {
  if (tx.hash)
    tx.hash = Bitcoin.ImpExp.BBE.importHash( tx.hash );
  //else
  ...
  for( var j=0,oi; j<tx.out.length; j++ ) {
    ...
    if (oi.scriptPubKey != undefined)
      ...
      oi.address = oi.address || oi.Address;
      if (oi.address)
